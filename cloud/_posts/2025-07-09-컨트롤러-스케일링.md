...
description: "쿠버네티스에서 애플리케이션의 처리 용량을 늘리는 가장 기본적인 방법은 파드(Pod)의 수를 늘리는 것이다 동일한 애플리케이션이 동작하는 여러 개의 파드를 레플리카(Replica)라고 부르며, 이 레플리카들은 클러스터의 여러 노드에 분산 배치되어 부하를 나누어 처리한다"
---
