...
tags: kvm qemu libvirt terraform
image: ''
mermaid: true
---
...
main.tf 파일이 이미 존재한다면 ssh 키를 넣어서 나중에 외부 접속을 비밀번호 없이 할 수 있다
...
tf 파일 실행시 권한 오류 해결 https://github.com/dmacvicar/terraform-provider-libvirt/issues/1163
...
❗**중요:** 마지막 `ensurepath` 명령어 실행 후, **터미널을 껐다가 다시 켜야** 시스템이 `pipx` 명령어를 인식
