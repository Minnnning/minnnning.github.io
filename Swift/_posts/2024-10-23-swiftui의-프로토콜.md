...
description: "스위프트를 학습하다보면 많은 프로토콜들이 나온다 클래스, 구조체등에서 공통된 속성이나 메서드를 정의하기 위한 구조이다 또한 스위프트에서는 다중 상속을 지원하지 않지만, 한 클래스 또는 구조체가 여러 프로토콜을 사용해서 다중 상속과 유사한 동작을 구현할 수 있다 스위프트에서 사용하는..."
---
