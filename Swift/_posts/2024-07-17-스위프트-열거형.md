...
description: "스위프트의 구조체는 struct 키워드를 사용한다 클래스와 매우 비슷한 구조를 가진다 두개의 차이점은 값 타입과 참조 타입이라는 것이 다르다 구조체는 값 타입, 클래스는 참조 타입이다 구조체는 인스턴스가 복사되거나 메서드에 전달될 때 인스턴스의 실제 복사본이 생성되면서 원본객체가..."
---
